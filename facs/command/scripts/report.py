...
class ReportCommand(AbstractCommand):
    def __init__(self, ods_bo):
        self.__ods_bo = ods_bo
    ...
    def get_commands(self):
        ...
        group.add_command(click.Command(
            name='create', help='initiate a workbook (ods format).',
        ...
        group.add_command(click.Command(
            name='update', help='update a workbook (update/add a sheet)',
    ...
    def create(self, csv_file, sheetname, outdir):
        ...
        if os.path.exists(out_workbook):
            raise ValueError('Output file {} already exist. Aborting to not overwrite.'.format(out_workbook))

        book = self.__ods_bo.get_book()
        book = self.__ods_bo.add_sheet(book, sheetname, csv_file)
        book.save(out_workbook)
        print('[+] Saved the report in {}'.format(out_workbook))

    def update(self, csv_file, workbook, sheetname):
        book = self.__ods_bo.get_book(workbook)

        sheet = self.__ods_bo.get_sheet_by_name(book, sheetname)
        if sheet is None:
            print('[+] Adding new sheet {}'.format(sheetname))
            book = self.__ods_bo.add_sheet(book, sheetname, csv_file)
        else:
            print('[+] Updating sheet {}'.format(sheetname))
            book = self.__ods_bo.update_sheet(book, sheet, csv_file)
